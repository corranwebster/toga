...
``` toml
[project.entry-points."toga.core.backends.toga_iOS"]
...
But if you have multiple implementations for different backends, you should do the following:
...
You then might write the interface for a new `BitmapView` widget:

[my_app.widgets.bitmap_view]
``` python
from toga import Widget
from my_app.factory import factory

class BitmapView(Widget):

    def create(self):
        self._impl = factory.BitmapView(self)
```

Then you might write the implementation for macOS:

[my_app.cocoa.widgets.bitmap_view]
``` python
...
[my_app.gtk.widgets.bitmap_view]
``` python
...
[project.entry-points."toga.my_app.backends.toga_cocoa"]
BitmapView = my_app.cocoa.bitmap_view:BitmapView

[project.entry-points."toga.my_app.backends.toga_gtk"]
...
## Adding an Implementation of a Third-Part Widget for a Backend
...
[project.entry-points."toga.my_app.backends.toga_qt"]
...
In the extreme, you could create your own factory instance and monkeypatch `toga.platform` with your own.
